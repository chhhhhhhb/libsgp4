...
namespace libsgp4
{
    ...
    OrbitalElements::OrbitalElements(const Tle &tle)
    {
         ...
         */
        mean_anomoly_ = tle.MeanAnomaly(false);                      // rad
        ascending_node_ = tle.RightAscendingNode(false);             // rad
        argument_perigee_ = tle.ArgumentPerigee(false);              // rad
        eccentricity_ = tle.Eccentricity();                          // 无
        inclination_ = tle.Inclination(false);                       // rad
        mean_motion_ = tle.MeanMotion() * kTWOPI / kMINUTES_PER_DAY; // rad/min
        bstar_ = tle.BStar();                                        // ER^-1
        epoch_ = tle.Epoch();
